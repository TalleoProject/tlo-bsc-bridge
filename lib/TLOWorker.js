...
var lastBlockFound, lastTopBlock = -1, TLOTransactions = {}, BSCAddresses = {};
            ...
            apiInterfaces.rpcWallet('getTransaction', {'transactionHash': txHash}, function (error, result) {
                ...
                if (result.transaction.blockIndex > 0) {
                    if (result.transaction.blockIndex > lastBlockFound) {
                        global.redisClient.hset('tlo-bsc-bridge:talleo', 'lastBlockFound', result.transaction.blockIndex, function (error1) {
                            if (error1) {
...
function confirmTalleoTransaction(txHash) {
    ...
    setTimeout(async function() {
        ...
        if (txConfirmations >= (global.config.talleo.confirmations || 10)) {
            ...
            global.redisClient.hget('tlo-bsc-bridge:talleo', 'conversion' + txHash, function (error, result) {
                ...
                if (result === null && TLOTransaction.amount > 100) {
                    var web3 = new Web3(new Web3.providers.WebsocketProvider(global.config.binance.wssNode));
                    web3.eth.accounts.privateKeyToAccount(global.config.binance.ownerKey);
                    fs.readFile(global.config.binance.contractABI, { encoding: 'utf-8' }, function (error1, data) {
                        if (error1) {
                            log('error', logTalleo, 'Error "%s" while trying to read contract ABI from file "%s"', [error1, global.config.binance.contractABI]);
                            return;
                        }
                        var tokenContract = new web3.eth.Contract(JSON.parse(data), global.config.binance.contractAddress);
                        var BSCAddress = payment_id_to_bsc(TLOTransaction.paymentId);
                        if (!Web3.utils.isAddress(BSCAddress)) {
                            log('error', logTalleo, 'Invalid address "%s" in conversion request', [BSCAddress]);
                            return;
                        }
                        if (BSCAddress.toLowerCase() == global.config.binance.ownerAddress.toLowerCase()) {
                            log('error', logTalleo, 'Trying to send tokens to owner address');
                            return;
                        }
                        if (BSCAddress.toLowerCase() == '0x0000000000000000000000000000000000000000') {
                            log('error', logTalleo, 'Trying to send tokens to NULL address');
                            return;
                        }
                        if (global.config.binance.blacklist) {
                            for (var i = 0; i < global.config.binance.blacklist.length; i++) {
                                if (global.config.binance.blacklist[i].toLowerCase() == BSCAddress.toLowerCase()) {
                                    log('error', logTalleo, 'Trying to send tokens to blacklisted address');
                                    return;
                                }
                            }
                        }
                        if (BSCAddresses[BSCAddress.toLowerCase()]) {
                            var lastConversion = BSCAddresses[BSCAddress.toLowerCase()];
                        ...
                        BSCAddresses[BSCAddress.toLowerCase()] = Date.now();
                        var account = web3.eth.accounts.wallet.add(global.config.binance.ownerKey);
                        web3.eth.getBlock("latest", false, (error2, result2) => {
                            ...
                            if (_gasLimit < global.config.binance.gas) {
                              _gasLimit = global.config.binance.gas;
                            }
                            if (_gasLimit > (global.config.binance.gas * 3)) {
                              _gasLimit = global.config.binance.gas * 3;
                            }
                            web3.eth.getGasPrice(async function (error3, _gasPrice) {
                                ...
                                var _hex_gas = web3.utils.toHex((global.config.binance.gas || 300000).toString());

                                var ownerBalance = await web3.eth.getBalance(account.address);
                                log('info', logTalleo, 'Gas price %s BNB, gas limit %s', [(_gasPrice / 1e18).toFixed(9), _gasLimit]);
                                var requiredBalance = (_gasPrice * _gasLimit);
                                log('info', logTalleo, 'Current balance %s BNB, required balance %s BNB!', [(ownerBalance / 1e18).toFixed(18), (requiredBalance / 1e18).toFixed(18)]);
                                if (ownerBalance < requiredBalance) {
                                    log('error', logTalleo, 'Binance wallet does not have enough balance!');
                                    setTimeout(function() {
                                        confirmTalleoTransaction(txHash);
                                    }, 3600000);
                                    return;
                                ...
                                web3.eth.getTransactionCount(global.config.binance.ownerAddress, 'pending').then(
                                nonce => {
                                    ...
                                    {
                                        nonce: _hex_nonce,
                                        from: global.config.binance.ownerAddress,
                                        to: global.config.binance.contractAddress,
                                    ...
                                    }, global.config.binance.ownerKey, function (error4, result4) {
                                        if (error4) {
                                        ...
                                        web3.eth.sendSignedTransaction(result4.rawTransaction, function (error5, result5) {
                                            ...
                                            global.redisClient.hset('tlo-bsc-bridge:talleo', 'conversion' + txHash, hash, function (error7, result7) {
                                                var amount = TLOTransaction.amount / 100;
                                                if (error7) {
                                                    log('error', logTalleo, 'Error "%s" while trying to store conversion request with TLO hash "%s", BSC hash "%s", recipient %s and amount %s TLO', [error7, txHash, hash, BSCAddress, amount.toFixed(2)]);
                                                    return;
                                                }
                                                log('info', logTalleo, 'Conversion request with recipient %s and amount %s TLO sent with hash %s', [BSCAddress, amount.toFixed(2), hash]);
                                            });
                                        });
                                    });
                                });
...
(function init() {
    "use strict";
    global.redisClient.hget('tlo-bsc-bridge:talleo', 'lastBlockFound', function (error, result) {
        if (error) {
            lastBlockFound = 0;
        } else {
